...
const getNameFromExpression = expression => {
  ...
  const dotted = properties.join('.').replace(/\.[[]/g, '[');

  return createMemberExpression(dotted);
};
...
const checkEvaluation = (stateOpts, path, argsArray, loc) => {
  ...
  argsArray = argsArray.map(item => item.value).filter(item => item);
...
const getNodeNames = (node, isVal) => {
  return node.arguments.map(argument => {
    if (argument.type === 'MemberExpression') {
      return getNameFromExpression(argument);
