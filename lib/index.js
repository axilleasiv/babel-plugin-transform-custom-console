...
const isLog = (callee, name = 'console') => {
  if (
    callee.object &&
...
const getNodeName = node => {
  if (node.arguments[0].type === 'MemberExpression') {
    return getNameFromExpression(node.arguments[0]);
  } else {
    ...
    return {
      ...
      visitor: {
        ...
        CallExpression(path, state) {
          ...
          if (node.arguments.length && isLog(node.callee, consoleName)) {
            const callee = node.callee;
            const line = node.loc.end.line;
            const rel = state.opts.rel;
            const name = getNodeName(node);

            console.log('Voila!', name);
