...
const getConsoleStatement = (
  ...
  if (type !== 'log') {
    consoleArgs.push(t.booleanLiteral(true));
  }
...
const getReferences = (exprPath, name, propName) => {
  ...
  if (callInScope) {
    ...
    if (parts.length > 1) {
      ...
      if (path.parentKey === 'declarations') {
        exprs = path.get('init.expressions').filter(expr => {
      ...
      path.traverse(
        {
          ...
          Identifier(path, state) {
            ...
            if (name !== idName) {
              if (bindings[idName]) {
                const callLocations = getReferences(exprPath, idName);
                locations = locations.concat(callLocations);
              }
            }
          }
        ...
        CallExpression(path, state) {
          ...
          if (args.length) {
            ...
            if (hasLog) {
              ...
              if (!isOnValArgs(state.opts, path, names, loc)) {
                if (args.length === 5) {
                  args.pop();
                }
              }
