  ...
  argsArray = argsArray
    .map(item => {
      if (item.type === 'MemberExpression') {
        return getNameFromMemberExpression(item, true);
      } else {
        return item.value;
      }
    })
...
const getNodeNames = (node, isVal) => {
  return node.arguments.map(argument => {
    if (argument.type === 'CallExpression') {
      return t.stringLiteral(getNameFromCallExpression(argument));
    } else if (argument.type === 'MemberExpression') {
      return t.stringLiteral(getNameFromMemberExpression(argument, true));
    } else if (isVal && argument.type === 'ArrayExpression') {
      const firstElem = argument.elements[0];
      if (firstElem.type === 'MemberExpression') {
        return t.stringLiteral(getNameFromMemberExpression(firstElem, true));
