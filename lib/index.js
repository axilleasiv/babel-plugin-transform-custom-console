...
const getDetectLoopStatement = (
  ...
  const callExpression = t.callExpression(
    t.memberExpression(t.identifier(stateOpts.consoleName), t.identifier(consoleType)),
    args
  );
...
const getCallExpression = (origNode, stateOpts, path, log) => {
  ...
  switch (type) {
    case 'ExpressionStatement':
      ...
      /* TODO: 'TypeError: Cannot read property \'split\' of undefined\n    at getReferences lib/index.js:123:22) */
...
const hasLogExpression = node => {
  if (
    ...
    isCommentLog(node.trailingComments[0].value)
  ) {
...
module.exports = {
  ...
  //TODO: what about console.log statements?
  // it fires the CallExpression below
  trace() {
    return {
      name: 'trace',
      visitor: {
        ...
        CallExpression(path, state) {
          ...
          if (path.node.console) {
            return;
          }

          const traceCall = getTraceStatement(
            state.opts,
            path.node.loc,
            traces.CALL,
            t.arrayExpression([])
        ...
        Function(path, state) {
          ...
          const traceEnter = getTraceStatement(
            state.opts,
            path.node.loc,
            traces.ENTER,
            t.arrayExpression(path.node.params)
          );
          //maybe to remove params from exit
          const traceExit = getTraceStatement(
            state.opts,
            path.node.loc,
            traces.EXIT,
            t.arrayExpression(path.node.params)
          ...
          //todo: directives on blockStatement
            ...
            t.catchClause(
              ...
              t.blockStatement([
                getTraceStatement(
                  ...
                  path.node.loc,
                  traces.EXCEPTION,
                  t.arrayExpression([t.identifier('e')]) //maybe to add params here also
                ),
              ])
            ),
            t.blockStatement([traceExit])
          );

          path.node.body.body = [traceEnter, tryStatement];
        },
      },
    };
  },
...
const getTraceStatement = (
  stateOpts,
  location,
  type,
  scopeValues // arrayExpression args, params, [error] for exception
) => {
  const consoleType = 'trace';

  const startLine = t.numericLiteral(location.start.line);
  const startColumn = t.numericLiteral(location.start.column);

  const endLine = t.numericLiteral(location.end.line);
  const endColumn = t.numericLiteral(location.end.column);

  const locationStart = t.arrayExpression([startLine, startColumn]);
  const locationEnd = t.arrayExpression([endLine, endColumn]);
  const locationArgument = t.arrayExpression([locationStart, locationEnd]);
  ...
  const callExpression = t.callExpression(
    t.memberExpression(
      ...
      t.identifier(consoleType)
    ),
    args
  );

  callExpression.loc = location;
  //TODO: add these to the other console options
  callExpression.console = consoleType;
