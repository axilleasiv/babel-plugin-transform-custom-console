...
function getConsoleStatement(node, name, line, rel) {
  var statement = t.expressionStatement(
    t.callExpression(
      t.memberExpression(t.identifier(name), t.identifier('log')),
      [node, t.numericLiteral(line || node.loc.end.line), t.stringLiteral(rel)]
    )
  );
...
function logThroughComments(node, path, state) {
  ...
  if (log) {
    path.replaceWith(
      getCallExpression(node, state.opts.consoleName, path, log, state.rel)
    );
...
function isReplFile(state) {
  var dir = Path.dirname(state.opts.fileName);
  var rel = Path.relative(dir, state.file.opts.filename);
  ...
  return {
    visitor: {
      ...
      BinaryExpression: function BinaryExpression(path, state) {
        ...
        if (path.parentKey === 'expression') {
          path.replaceWith(
            getConsoleStatement(node, state.opts.consoleName, state.rel)
          );
        }
      }
    }
