      ...
      path.traverse(
        {
          ...
          Identifier(path, state) {
            ...
            if (
              ...
              !['argument', 'arguments'].includes(path.parentKey)
            ) {
              return;
            }
...
const getCallExpression = (origNode, stateOpts, path, log) => {
  ...
  switch (type) {
    case 'ExpressionStatement':
      ...
      // callExpression scoped locations
      // TODO: check to ignore exprssions that returning primitive values
      if (node.type === 'CallExpression') {
        try {
          let name = getNameFromExpression(node, true, false);
          extraLoc = getReferences(path, name);
        } catch (err) {
          // TODO: send error in logger;
        }
      }
        ...
        CallExpression(path, state) {
          ...
          if (node.arguments.length && isLog(node.callee)) {
            ...
            try {
              ...
              const callNames = names
                .filter(name => name.value.includes('()'))
                .map(name => stripParen(name.value));

              if (callNames.length) {
                callNames.forEach(name => {
                  extraLocs.push(
                    prepareLoc(node.loc, getReferences(path, name))
                  );
                });
              }
            } catch (err) {}
