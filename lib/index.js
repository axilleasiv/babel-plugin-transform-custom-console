...
const getConsoleStatement = (logsExpr, stateOpts, loc, namesExpr, type) => {
  ...
  const callExpression = t.callExpression(
    ...
    [
      logsExpr,
      t.arrayExpression([startLine, endLine]),
...
// Object.keys(exprPath.scope.bindings).forEach(key => {
//   const binding = exprPath.scope.bindings[key];
//   const { referenced, referencePaths, path } = binding;

//   if (referenced) {
//   }
// });
// const { bindings } = exprPath.scope;
...
// const isAncestor = exprPath.isAncestor(ref);
// const isDesc = ref.isDescendant(exprPath);
...
const getReferences = (exprPath, name, propName) => {
  ...
  if (callInScope) {
    ...
    if (parts.length > 1) {
      ({ path } = callInScope);
      const exprs = path.get('init.expressions').filter(expr => {
        ...
        if (exprs.length === 1) {
          ...
          switch (path.type) {
            case 'ObjectExpression':
              ...
              node.properties.some(prop => {
                ...
                if (arg.name === propertyName) {
                  if (prop.body) {
                    ...
                    loc = arg.value ? arg.value.loc : arg.loc;
                  }
                  return true;
                }
              ...
              locations.push(offsetLines);
              break;
            case 'NewExpression':
              locations.push([loc.start.line, loc.end.line]);
      ...
      path.traverse(
        {
          CallExpression(path, state) {
            ...
            const callLocations = getReferences(exprPath, name);
            locations = locations.concat(callLocations);

            // if (node.callee.name) {
            //   const callLocations = getReferences(exprPath, node.callee.name);
            //   locations = locations.concat(callLocations);
            // }
          },
          NewExpression(path) {
          ...
          Identifier(path, state) {
            ...
            console.log('-------->>>', path.parentKey, idName);
            if (!['argument', 'arguments'].includes(path.parentKey)) {
              return;
            }

            if (name !== idName) {
              let call;
              if ((call = bindings[idName])) {
                const { path } = call;
                const { loc } = path.node;
                locations.push([loc.start.line, loc.end.line]);
                if (call.referenced) {
...
const getCallExpression = (origNode, stateOpts, path, log) => {
  ...
  switch (type) {
    case 'ExpressionStatement':
      ...
      // TODO: add all checks and console.log call exprs
      // If type of log returns expression object/array/function
      // the logs that are returning primitive values, it is better
      // not to check for them
      // callExpression scopes only
      let name = getNameFromExpression(node, true, false);
      const loc = getReferences(path, name);
      console.log('========>>>>', loc);
